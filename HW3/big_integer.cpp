...
big_integer::big_integer()  {
    value = SharedVector<uint32_t >(0);
    sign = 1;
}

big_integer::big_integer(int a) : sign(a < 0 ? -1 : 1) {
    value = SharedVector<uint32_t >((uint32_t)(llabs(a)));
}

big_integer::big_integer(uint32_t a) : sign(1) {
    value = SharedVector<uint32_t >(a);
}

big_integer::big_integer(int s, const std::vector<uint32_t> &v) {
    sign = s;
    value = SharedVector<uint32_t>(v);
    this->clean();
}

big_integer::big_integer(int s, const SharedVector<uint32_t> &v) {
    sign = s;
    value = SharedVector<uint32_t>(v);
    this->clean();
}
...
big_integer operator+(const big_integer &a, const big_integer &b) {
    if (a.sign != b.sign) { // vector > shared vector
        return a - (-b);
    }
    int carry = 0;
    size_t newsz = std::max(a.size(), b.size());
    uint64_t sum = 0;
    SharedVector<uint32_t> res(newsz, 0);
...
big_integer operator-(const big_integer &a, const big_integer &b) {
    ...
    int carry = 0;
    SharedVector<uint32_t> res = a.value;
    uint64_t sub = 0;
    size_t asz = a.size();
...
big_integer operator*(const big_integer &a, const big_integer &b) { //vector -> SharedVector
    SharedVector<uint32_t> x(a.size() + b.size() + 1, 0);
...
big_integer big_integer::from_bit() const {
    ...
    SharedVector<uint32_t> inv(value);
    size_t sz = inv.size();
...
big_integer binary_f(const big_integer &a, const big_integer &b, F f) {
    ...
    SharedVector<uint32_t> res;
    big_integer a_bit(a.to_bit());
    big_integer b_bit(b.to_bit());
    for (size_t i = 0; i < sz; ++i) {
        res.push_back(f(a_bit.bit_digit(i), b_bit.bit_digit(i)));
    }
    bool a_neg = (a.sign == -1);
    bool b_neg = (b.sign == -1);
...
big_integer operator<<(big_integer a, int d) {
    ...
    SharedVector<uint32_t> x(newsz, 0);
    size_t asz = a.size();
    for (size_t i = 0; i < asz; ++i) {
        if (move) {
...
big_integer operator>>(big_integer a, int d) {
    ...
    size_t move = d % block_size;
    big_integer a_bit = a.to_bit();
    SharedVector<uint32_t> x(a.size() - delta, 0);
    size_t xsz = x.size();
...
big_integer big_integer::short_divide(uint32_t a) const {
    ...
    big_integer ans(1, SharedVector<uint32_t>(x));
    x.clear();
    ans.clean();
    return ans;
}
...
big_integer long_divide(const big_integer &a, const big_integer &b) {
    ...
    if (a.abs_() < b.abs_()) {
        ...
        res = big_integer(a.sign * b.sign, SharedVector<uint32_t>(1));
    } else if (b.size() == 1) {
        ...
        big_integer Bn_1(1, SharedVector<uint32_t>(B[n - 1]));
        for (size_t i = m; i-- > 0;) {
            Bm >>= block_size;
            std::vector<uint32_t > v = {A[n + i - 1], A[n + i]};
            big_integer q(1, SharedVector<uint32_t >(v));
            q /= Bn_1;
            if (q.size() > 1) {
                q.value = SharedVector<uint32_t >(MX32);
                q.sign = 1;
            }
            A -= Bm * q;
            while (A < 0) {
